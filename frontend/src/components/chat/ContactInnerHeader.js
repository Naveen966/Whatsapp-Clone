import React, { useState } from "react";
import Settings from "./Settings";

function ContactInnerHeader() {
  const [property, newProperty] = useState("none");
  const settingsMenu = () => {
    property == "none" ? newProperty("block") : newProperty("none");
  };

  const logo = document.getElementById("yourLogo");
  return (
    <>
      <div
        style={{
          display: "flex",
          backgroundColor: "#5555",
          alignItems: "center",
          justifyContent: "space-between",
        }}
      >
        <div className="yourLogo" onClick={settingsMenu}>
          <img
            style={{ width: "3vw", borderRadius: "50%" }}
            src="data:image/png;base64,"
          />
        </div>
        <div className="container" style={{ display: "flex" }}>
          <div className="status">
            <span className="material-symbols-outlined">donut_large</span>
          </div>
          <div className="search">
            <span className="material-symbols-outlined">search</span>
          </div>
        </div>
      </div>

      <Settings displayProperty={property} />
    </>
  );
}

export default ContactInnerHeader;
